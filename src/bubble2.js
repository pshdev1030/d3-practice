...
function App() {
  ...
  useEffect(() => {
    ...
    function getMinLength(text, r) {}

    const canvas = select(canvasRef.current);
    const mapData = hierarchy(data, (node) => node).sum(
      (node) => node["value"]
    );

    const createPack = pack().size([750, 750]).padding(5);
    ...
    treeElement
      ...
      .attr("r", (node) => node["r"])
      .attr("fill", (node) =>
        node["data"]["color"] ? node["data"]["color"] : randomColor()
      );
